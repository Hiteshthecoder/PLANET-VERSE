...
class _EnemyPlanets extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final List<Planet> _availablePlanets = _gameData.enemyPlanetsFor(_player.ruler);
            ...
            children: [
              Container(
                ...
                child: CarouselSlider.builder(
                  ...
                  itemBuilder: (BuildContext context, int index, _) => GestureDetector(
                    onTap: () {
                      if (!_player.canAttack) {
                        Utility.showToast('Nahh !! Too tired after the last one');
                      } else if (_availablePlanets[index].inWar) {
                        Utility.showToast('That one is already under attack by someone');
                      } else {
                        Navigator.of(context).pop();
                        Navigator.of(context).pushNamed(AttackScreen.route, arguments: {
                          'planet': _availablePlanets[index],
                          'attacker': Provider.of<Player?>(context, listen: false)!,
                        });
                      }
                    },
                    child: _planetCard(describeEnum(_availablePlanets[index].name)),
                  ),
                ),
              ),
              const Align(alignment: Alignment.centerLeft, child: Icon(Icons.arrow_left)),
              const Align(alignment: Alignment.centerRight, child: Icon(Icons.arrow_right)),
            ],
          );
  }
}
      ...
      child: Row(
        children: List.generate(
          ...
          (index) => _MyForceCard(
            ...
            quantity: _player.militaryShipCount(_player.ships.keys.toList()[index]),
          ),
        ),
      ),
    );
      ...
      child: Column(
        ...
        children: [
          LayoutBuilder(builder: (context, constraints) {
            return Container(
              height: min(max(size.width, size.height) / 10, constraints.maxWidth),
              width: min(max(size.width, size.height) / 10, constraints.maxWidth),
          ...
          Text(quantity.toString(), style: Theme.of(context).textTheme.headline6!.copyWith(fontWeight: FontWeight.bold))
        ],
      ),
    );
  }
