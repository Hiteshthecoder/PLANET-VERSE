          ...
          children: [
              ...
              child: TabBarView(
                children: List.generate(
                  ...
                  (index) => AttackShipInfo(
                    attackShip: kAttackShipsData[_player.ships.keys.toList()[index]]!,
                  ),
                ),
              ),
            ),
            TabBar(
                ...
                tabs: List.generate(
                    _player.ships.length,
                    (index) => Tab(
                          text: describeEnum(_player.ships.keys.toList()[index]).inCaps,
                        ))),
          ],
        ),
      ));
...
class _TransactionButton extends StatelessWidget {
  const _TransactionButton({Key? key, required this.text, required this.onTap}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Expanded(
      ...
      child: Container(
        ...
        child: Text(text, style: Theme.of(context).textTheme.headline6!.copyWith(fontWeight: FontWeight.bold)),
      ),
    ));
  }
}
              ...
              child: Row(children: [
                Expanded(
                  child: Container(
                    ...
                    margin: const EdgeInsets.all(4),
                    child: Image.asset('assets/img/ships/attack/${describeEnum(attackShip.type)}.png'),
                  ),
                ),
                Expanded(
                  flex: 3,
...
class _MilitaryDialogStatsBox extends StatelessWidget {
  const _MilitaryDialogStatsBox({Key? key, required this.header, required this.value}) : super(key: key);
            ...
            return constraints.maxHeight - 28.sp > 0
                ? Column(
                    children: [
                      Text(
                        header,
                        style: const TextStyle(fontWeight: FontWeight.w600, color: Colors.white54),
                      ),
                      Expanded(
                        child: Center(
                          child: Text(
