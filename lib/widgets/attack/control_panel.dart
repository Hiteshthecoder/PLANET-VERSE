...
import 'package:planet_verse/services/player/player.dart';
...
class _ControlPanelState extends State<ControlPanel> with TickerProviderStateMixin {
  ...
  Future<void> showOverlay(String attackPoint, String defensePoint) async {
    ...
    final _renderBox = _attackButtonKey.currentContext!.findRenderObject()! as RenderBox;
                    ...
                    children: [
                      Text(
                        ...
                        style: Theme.of(context)
                            .textTheme
                            .headline5!
                            .copyWith(color: Colors.lime, fontWeight: FontWeight.bold),
                      ),
                      Text(
                        defensePoint,
                        style: Theme.of(context)
  ...
  Widget _baseCard({required Widget child}) {
    return Expanded(
      child: Container(
        ...
        decoration: BoxDecoration(color: Colors.black45, borderRadius: BorderRadius.circular(4.sp)),
        child: child,
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final FormationProvider _formationProvider = Provider.of<FormationProvider>(context, listen: false);
    ...
    List<int> bestFormationForAttacker() {
      ...
      for (final formation in _formationProvider.formations) {
        ...
        final likeablilityFactor = _planet.effectFromDamageOutput(damageOutputs);
    ...
    Map<String, bool> _canDamage() {
      ...
      int likabilityDefense = 0; // Stores the Max Damage points the Planet will score
      for (final formation in _formationProvider.formations) {
        final damageOutputs = _planet.damageOutputForFormation(formation);
        final likeablilityFactor = _attacker.effectFromDamageOutput(damageOutputs);
        if (likeablilityFactor > likabilityDefense) {
          likabilityDefense = likeablilityFactor;
        }
      }
      int likabilityAttack = 0; // Stores the Max Damage points the Attacker can score
      for (final formation in _formationProvider.formations) {
        final damageOutputs = _attacker.damageOutputForFormation(formation);
        final likeablilityFactor = _planet.effectFromDamageOutput(damageOutputs);
    ...
    void _goToConclusionScreen(String message) {
      ...
      Navigator.pushReplacementNamed(context, AttackConclusionScreen.route, arguments: message);
    }
                          ...
                          children: [
                            Center(
                              child: CarouselSlider(
                                ...
                                items: List.generate(
                                  pow(kAttackShipsData.length, kAttackShipsData.length) as int,
                                  (index) => Center(
                                    child: FittedBox(
                                      child: Text(
                                        '${index + 1}',
                            ...
                            Center(
                              child: Row(
                                ...
                                children: [
                                  GestureDetector(
                                      onTap: () {
                                        formationCarouselController.previousPage(
            ...
            child: SizedBox(
              ...
              child: Padding(
                ...
                child: ElevatedButton(
                  ...
                  style: ButtonStyle(
                    backgroundColor: MaterialStateProperty.all<Color>(Palette.maroon),
                  ),
                  onPressed: () {
                    // TODO : All the Attack related logic is here

                    final playerFormation = _formationProvider.currentFormation;
                    final computerFormation =
                        _isPlayerAttacker ? bestFormationForDefender() : bestFormationForAttacker();
                    final attackFormation = _isPlayerAttacker ? playerFormation : computerFormation;
                    final defenseFormation = !_isPlayerAttacker ? playerFormation : computerFormation;

                    final _initialAttackShips = List.from(_attacker.ships.values).fold(0, (dynamic p, c) => p + c);
                    final _initialDefenseShips = List.from(_planet.ships.values).fold(0, (dynamic p, c) => p + c);

                    final damageOutputsDefense = _planet.damageOutputForFormation(defenseFormation);
                    final damageOutputsAttacker = _attacker.damageOutputForFormation(attackFormation);
                    final attackShipsLeft = _attacker.defendAgainstDamageOutput(damageOutputsDefense);
                    final defenseShipsLeft = _planet.defendAgainstDamageOutput(damageOutputsAttacker);
                    showOverlay('-${_initialAttackShips - attackShipsLeft} Ships',
                    ...
                    if (attackShipsLeft == 0) {
                      _goToConclusionScreen('The Planet Successfully defended itself');
                    } else if (defenseShipsLeft == 0) {
                      if (_overlayEntry != null) {
                        _overlayEntry!.remove();
                        _overlayEntry = null;
                      }
                      _gameData.changeOwnerOfPlanet(newRuler: _attacker.ruler, planetName: _planet.name);
                      if (_gameData.wonGame) {
                        ...
                        _goToConclusionScreen('The planet succumbed to its attacker');
                      }
                      ...
                      _goToConclusionScreen('The Attacker has retreated, Planet is Safe');
                      // attacker gives up
                    }
