...
class Battlefield extends StatelessWidget {
  ...
  Widget _attackShips(BoxConstraints constraints) {
    ...
    return Consumer<Player>(
      builder: (_, _attacker, __) {
        final double iconSize = constraints.maxHeight / _attacker.ships.length - 32.sp;
        return Column(
              ...
              child: SizedBox(
                ...
                child: Column(
                  children: [
                    ...
                    Text('${_attacker.ships[_attacker.ships.keys.toList()[index]]}'),
                  ],
                ),
              ),
            ),
  ...
  Widget _defenseShips(BoxConstraints constraints) {
    ...
    return Consumer<Planet>(
      builder: (_, _planet, __) {
        final double iconSize = constraints.maxHeight / _planet.ships.length - 32.sp;
        return Column(
          mainAxisSize: MainAxisSize.min,
          children: List.generate(
              ...
              child: SizedBox(
                ...
                child: Column(
                  children: [
                    ...
                    Text('${_planet.ships[_planet.ships.keys.toList()[index]]}'),
                  ],
                ),
              ),
            ),
    ...
    return Consumer<FormationProvider>(
      builder: (_, _formationProvider, ___) {
        return Expanded(
            child: CustomPaint(
                painter: _attackMode
                    ? AttackerFormationPainter(formation: _formationProvider.currentFormation)
                    : DefenderFormationPainter(formation: _formationProvider.currentFormation),
                child: Container()));
      },
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (_, constraints) {
      return Row(
        children: [_attackShips(constraints), _formationPaths(_isCurrentPlayerAttacker), _defenseShips(constraints)],
      );
    });
  }
}
