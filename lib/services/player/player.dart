...
import 'package:planet_verse/models/defense_ships_model.dart';
import 'package:planet_verse/models/planet_model.dart';
import 'package:planet_verse/models/ruler_model.dart';
import 'package:planet_verse/services/interaction.dart';
...
class Player extends ChangeNotifier with Stats, Military, Planets {
  ...
  Player({required this.ruler, required List<Planet> planets, required this.playerType}) {
    money = 10000;
    canAttack = true;
    planetsInit = planets;
    statsInit();
  ...
  int get militaryMight {
    ...
    for (final shipType in ships.keys.toList()) {
      militaryMight = kAttackShipsData[shipType]!.point * militaryShipCount(shipType);
    }
    return (militaryMight * (1 + (statValue(StatsType.military) * 0.0025))).toInt();
  }

  int get galacticPowerIndex {
    return statValue(StatsType.culture) * 5 + militaryMight + planets.length * 50 + planetsGPIBonus() * 30;
  }
  ...
  void nextTurn() {
    ...
    final _militaryEffect = min(0, statValue(StatsType.propoganda) * 3 - militaryMoraleImpact);
    ...
    final _culturalEffect =
        min<int>(0, statValue(StatsType.culture) - min<int>(100, planets.length * (Random().nextInt(5) + 10))) * 5;
    final _baseMorale =
        _militaryEffect + _luxuryGains + _culturalEffect + min<int>(250, (galacticPowerIndex * 0.5).ceil());
  ...
  void autoBuyMilitary(int moneyAllotted) {
    ...
    for (int i = 0; i < _attackShips.length; i++) {
      final shipsToBuy = ((moneyAllotted * budgets[i]) / _attackShips[i].cost).floor();
  ...
  void autoBuyDefense(int moneyAllotted) {
    ...
    myPlanets.sort((a, b) => Comparable.compare(a.militaryMight, b.militaryMight));

    for (int i = 0; i < myPlanets.length; i++) {
      autoBuyDefenseShipsForPlanet(
          moneyAllotted: (moneyAllotted * planetBudgetAllotment[i] / 100).floor(), planetName: myPlanets[i].name);
    }
  }
  ...
  void autoIncreaseStats() {
    ...
    if (statValue(StatsType.luxury) < 95 && statValue(StatsType.luxury) < planets.length * 20) {
      increaseStat(StatsType.luxury);
      increaseStat(StatsType.luxury);
      increaseStat(StatsType.luxury);
    }
    if (statValue(StatsType.military) < 95 && statValue(StatsType.military) < planets.length * 20) {
  ...
  void autoDecreaseStats() {
    ...
    if (statValue(StatsType.luxury) > planets.length * 20 && statValue(StatsType.luxury) > 15) {
      decreaseStat(StatsType.luxury);
      decreaseStat(StatsType.luxury);
      decreaseStat(StatsType.luxury);
    }
    if (statValue(StatsType.military) > 15 && statValue(StatsType.military) > planets.length * 20) {
  ...
  void autoBuyDefenseShipsForPlanet({required int moneyAllotted, required PlanetName planetName}) {
    final budgets = [0.5, 0.0, 0.5]..shuffle();
    final _defenseShips = kDefenseShipsData.values.toList();
    for (int i = 0; i < _defenseShips.length; i++) {
      final shipsToBuy = ((moneyAllotted * budgets[i]) / _defenseShips[i].cost).floor();
      for (int j = 0; j < shipsToBuy; j++) {
  ...
  void autoBuyUpgradeForPlanet({required int moneyAllotted, required PlanetName planetName}) {
    int moneyLeft = moneyAllotted;
    for (final upgradeType in UpgradeType.values) {
      if (planetUpgradeAvailable(type: upgradeType, name: planetName) && moneyLeft > kUpgradesData[upgradeType]!.cost) {
  ...
  int effectFromDamageOutput(List<int> damageOutputs) {
    ...
    for (int i = 0; i < damageOutputs.length; i++) {
      final shipsLost = damageOutputs[i] ~/ kAttackShipsData[ships.keys.toList()[i]]!.health;
      shipDestroyed[ships.keys.toList()[i]] = min(militaryShipCount(ships.keys.toList()[i]), shipsLost);
    }
  ...
  List<int> damageOutputForFormation(List<int> formation) {
    ...
    final damageOutput =
        List.generate(formation.length, (index) => 0); // What Damage will enemy ship at pos[i] reciveve
    for (int i = 0; i < formation.length; i++) {
      ...
      damageOutput[formation[i]] +=
          (shipCount * shipAttackPower * (1 + (statValue(StatsType.military) * 0.025))).toInt();
    }
    return damageOutput;
  }
  ...
  void buyDefenseShip({required DefenseShipType type, required PlanetName name}) {
  ...
  void sellDefenseShip({required DefenseShipType type, required PlanetName name}) {
