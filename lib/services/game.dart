...
import 'package:planet_verse/models/attack_ships_model.dart';
import 'package:planet_verse/models/upgrade_model.dart';
...
class Game extends ChangeNotifier {
  ...
  void initPlayers(Ruler selectedRuler) {
    ...
    players.add(
      Player(
          planets: <Planet>[Planet(PlanetName.miavis), Planet(PlanetName.drukunides)],
          ruler: Ruler.morbo,
          playerType: selectedRuler != Ruler.morbo ? PlayerType.computer : PlayerType.human),
    );
    players.add(
      Player(
          planets: <Planet>[Planet(PlanetName.arth), Planet(PlanetName.musk)],
          ruler: Ruler.zapp,
          playerType: selectedRuler != Ruler.zapp ? PlayerType.computer : PlayerType.human),
    );
    players.add(
      Player(
          planets: <Planet>[Planet(PlanetName.jupinot), Planet(PlanetName.ocorix)],
          ruler: Ruler.ndnd,
          playerType: selectedRuler != Ruler.ndnd ? PlayerType.computer : PlayerType.human),
    );
    players.add(
      Player(
          planets: <Planet>[Planet(PlanetName.eno), Planet(PlanetName.hounus)],
          ruler: Ruler.nudar,
          playerType: selectedRuler != Ruler.nudar ? PlayerType.computer : PlayerType.human),
    );
  }
  ...
  Player get currentPlayer {
    return players.firstWhere((player) => player.playerType == PlayerType.human);
  }

  List<Player> get computerPlayers {
    return List<Player>.from(players.where((player) => player.playerType == PlayerType.computer));
  }
  ...
  void removeDeadPlayers() {
    ...
    players.removeWhere((player) {
      if (player.planets.isEmpty) {
        galacticNews.add('${describeEnum(player.ruler).inCaps} died in the last battle. His galatic empire is over ');
        return true;
      } else {
        return false;
      }
  ...
  void changeOwnerOfPlanet({required Ruler newRuler, required PlanetName planetName}) {
    galacticNews.add('${describeEnum(newRuler).inCaps} took over the Planet ${describeEnum(planetName).inCaps}');
    ...
    for (final computerPlayer in computerPlayers) {
      for (final rivalPlayer in players) {
        if (rivalPlayer.ruler != computerPlayer.ruler) {
          final diff = computerPlayer.galacticPowerIndex - rivalPlayer.galacticPowerIndex;
          final relation = relationBetweenRulers(computerPlayer.ruler, rivalPlayer.ruler);
          if (diff > godlyDifference) {
            // Player A is in very superior Position
            switch (relation) {
              case Relation.peace:
                if (chanceSucceeds(0.8)) {
                  updateRelation(computerPlayer.ruler, rivalPlayer.ruler, Relation.war);
                }
                break;
              case Relation.trade:
                if (chanceSucceeds(0.8)) {
                  updateRelation(computerPlayer.ruler, rivalPlayer.ruler, Relation.peace);
                }
                break;
              case Relation.war:
    ...
    for (final attacker in computerPlayers) {
      for (final defender in computerPlayers) {
        if (attacker.ruler != defender.ruler &&
            relationBetweenRulers(attacker.ruler, defender.ruler) == Relation.war &&
            attacker.canAttack) {
          ...
          if (chanceSucceeds(chance)) {
            ...
            final diffMilitaryMight = attacker.militaryMight - defendingPlanet.militaryMight;
            defendingPlanet.inWar = true;
            attacker.canAttack = false;
            if (diffMilitaryMight > godlyDifferenceMilitary) {
              attacker.destroyMilitary(Random().nextDouble() * 0.20); // 0-20% Military might be destroyed
              changeOwnerOfPlanet(newRuler: attacker.ruler, planetName: defendingPlanet.name);
              removeDeadPlayers();
              break;
              ...
              removeDeadPlayers();
              break;
            } else if (diffMilitaryMight > almostEqualsMilitary && chanceSucceeds(0.5)) {
  ...
  Map<String, Object>? computerAttacksCurrentPlayer() {
    for (final attacker in computerPlayers) {
      ...
      if (relationBetweenRulers(attacker.ruler, defender.ruler) == Relation.war && attacker.canAttack) {
        final chance = calculateChanceForAutoAttack(A: attacker, B: defender);
        if (chanceSucceeds(chance)) {
          final defenderPlanets = defender.planets;
          final selectedPlanetIndex = Random().nextInt(defenderPlanets.length);
          return {'ruler': attacker.ruler, 'planet': defender.planets[selectedPlanetIndex]};
        }
      }
    }
    return null;
  ...
  void updateRelation(Ruler A, Ruler B, Relation relation) {
    ...
    galacticNews
        .add('${describeEnum(A).inCaps} started ${describeEnum(relation).inCaps} with ${describeEnum(B).inCaps}');
  }

  // Each Player can perform only one Action with each player per turn
  // So you can't ask for war if started trading/peace etc
  String interactWithRival({required ChatOptions action, required Ruler A, required Ruler B}) {
    final _initialRelation = relationBetweenRulers(A, B);
    final chance = calculateChance(
        relation: _initialRelation,
        interactions: action,
        diffGPI: playerFromRuler(A).galacticPowerIndex - playerFromRuler(B).galacticPowerIndex);

    if (chanceSucceeds(chance)) {
      final map = yesEffectOfAction(relation: _initialRelation, interactions: action);
      ...
      final map = noEffectOfAction(relation: _initialRelation, interactions: action);
      if (_initialRelation != map['relation']) {
        updateRelation(A, B, map['relation'] as Relation);
      }
      notifyListeners();
